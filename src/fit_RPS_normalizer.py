...
class RPS_normalizer:
    """normalizer class that can fit a 2 componenent PCA model and
    then transform all the a,b coordinates from images into 'pigmentaiton'
    values which correspond to the RPS values
    """
    ...
    def fit(self) -> None:
        ...
        if (pca.components_[0][0] > 0) and (pca.components_[0][1] > 0):
            self.log.info(
                "inerverting PCA, previous pca eigenvector", pca.components_[0]
            )
    ...
    def transform(self) -> None:
        ...
        self.df["pigmentation"] = (
            self.df[["a", "b"]]
            .dropna()
            .apply(lambda x: self.pca.transform([[x.a, x.b]])[0][0], axis=1)
        )
    ...
    def save_example(self) -> None:
        ...
        plt.suptitle(
            "Representative median retinal background colors and their corresponding RPS",
            fontsize=24,
        )
