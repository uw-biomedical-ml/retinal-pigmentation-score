    ...
    with tqdm(total=n_val, desc="Validation round", unit="batch", leave=False) as pbar:
        for batch in loader:
            ...
            for i in range(n_image):
                ...
                mask_pred_img2 = torchvision.transforms.ToPILImage()(
                    mask_pred_sigmoid[i, ...]
                )
                mask_pred_img2 = mask_pred_img2.resize(
                    (n_ori_width, n_ori_height)
                ).convert("L")
...
def test_net(
    ...
    dir_checkpoint_1 = Path(
        __file__
    ).parent / "./Saved_model/train_on_{}/{}_savebest_randomseed_{}/G_best_F1_epoch.pth".format(
        dataset_train, job_name, 24
    )
    dir_checkpoint_2 = Path(
        __file__
    ).parent / "./Saved_model/train_on_{}/{}_savebest_randomseed_{}/G_best_F1_epoch.pth".format(
        dataset_train, job_name, 26
    )
    dir_checkpoint_3 = Path(
        __file__
    ).parent / "./Saved_model/train_on_{}/{}_savebest_randomseed_{}/G_best_F1_epoch.pth".format(
        dataset_train, job_name, 28
    )
    dir_checkpoint_4 = Path(
        __file__
    ).parent / "./Saved_model/train_on_{}/{}_savebest_randomseed_{}/G_best_F1_epoch.pth".format(
        dataset_train, job_name, 30
    )
    dir_checkpoint_5 = Path(
        __file__
    ).parent / "./Saved_model/train_on_{}/{}_savebest_randomseed_{}/G_best_F1_epoch.pth".format(
        dataset_train, job_name, 32
    )
    dir_checkpoint_6 = Path(
        __file__
    ).parent / "./Saved_model/train_on_{}/{}_savebest_randomseed_{}/G_best_F1_epoch.pth".format(
        dataset_train, job_name, 34
    )
    dir_checkpoint_7 = Path(
        __file__
    ).parent / "./Saved_model/train_on_{}/{}_savebest_randomseed_{}/G_best_F1_epoch.pth".format(
        dataset_train, job_name, 36
    )
    dir_checkpoint_8 = Path(
        __file__
    ).parent / "./Saved_model/train_on_{}/{}_savebest_randomseed_{}/G_best_F1_epoch.pth".format(
        dataset_train, job_name, 38
    )
    dir_checkpoint_9 = Path(
        __file__
    ).parent / "./Saved_model/train_on_{}/{}_savebest_randomseed_{}/G_best_F1_epoch.pth".format(
        dataset_train, job_name, 40
    )
    dir_checkpoint_10 = Path(
        __file__
    ).parent / "./Saved_model/train_on_{}/{}_savebest_randomseed_{}/G_best_F1_epoch.pth".format(
...
def M2_vessel_seg(cfg):
    ...
    if args.load:
        net_G.load_state_dict(torch.load(args.load, map_location=device))

        net_D.load_state_dict(torch.load(args.load, map_location=device))
