...
def quality_assessment(cfg):
    result_Eyepacs_path = "{}M1/results_ensemble.csv".format(
        cfg.results_dir
    )  # TODO replace this in some way?
    ...
    if cfg.quality_thresh == "good":
        usable = result_Eyepacs_[
            (result_Eyepacs_["Prediction"] == 1)
            & (result_Eyepacs_["softmax_bad"] < 0.25)
        ]["Name"].values
    if cfg.quality_thresh == "usable":
        usable = result_Eyepacs_[(result_Eyepacs_["Prediction"] == 1)]["Name"].values
    if cfg.quality_thresh == "all":
        usable = result_Eyepacs_[result_Eyepacs_["Prediction"].isin([1, 2])][
            "Name"
        ].values
    ...
    crops = pd.read_csv(
        cfg.results_dir + "M0/crop_info.csv",
        usecols=["Name", "centre_h", "centre_w", "radius"],
    )
    crops = crops[crops.apply(find_zeros, axis=1)]
    gq_crops = crops[crops["Name"].isin(gq)]

    gq_crops.to_csv(
        "{}/M1/Good_quality/image_list.csv".format(cfg.results_dir), index=False
    )
    bq_crops = crops[crops["Name"].isin(bq)]
    bq_crops.to_csv(
        "{}/M1/Bad_quality/image_list.csv".format(cfg.results_dir), index=False
    )
    ...
    if cfg.sparse == False:
        print(
            "copying good and bad quality images from M0/images into subsequent directories"
        )
