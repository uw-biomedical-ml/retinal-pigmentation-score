...
def optic_disc_centre(result_path, binary_vessel_path, artery_vein_path, cfg):
    ...
    for i in disc_cup_list:
        ...
        try:
            ...
            if disc_horizontal_width<(disc_.shape[0]/3) and disc_vertical_height<(disc_.shape[1]/3) and \
                 cup_width_centre<=np.max(disc_index_width) and cup_width_centre>=np.min(disc_index_width) and \
                 cup_height_centre<=np.max(disc_index_height) and cup_height_centre>=np.min(disc_index_height) and \
                 cup_vertical_height<disc_vertical_height and cup_horizontal_width<disc_horizontal_width:
                ...
                distance_ = np.sqrt(np.square(horizontal_distance)+np.square(vertical_distance)) # distance_ is the distance between the center of the disc_cup and the center of the image
