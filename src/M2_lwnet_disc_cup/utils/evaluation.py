...
def evaluate(logits, labels, n_classes, ignore_index=-100, fast=True):
    ...
    if n_classes == 1:
        ...
        all_probs_np = np.stack(
            [all_probs_0, all_probs_1, all_probs_2, all_probs_3], axis=1
        )
    ...
    if n_classes == 4:
        ...
        return roc_auc_score(
            all_targets_np, all_probs_np, multi_class="ovo", average="weighted"
        ...
        if fast == True:
            return fast_auc(all_targets_np > 0.5, all_probs_np), f1_score(
                all_targets_np > 0.5, all_preds_np
            )
        else:
            return roc_auc_score(all_targets_np, all_probs_np), f1_score(
